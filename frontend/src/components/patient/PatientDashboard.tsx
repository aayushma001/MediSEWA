...
// ✅ FIX 1: Default import (was named import { BookAppointment } — that caused the crash)
...
export const PatientDashboard: React.FC<PatientDashboardProps> = ({ patient, onLogout }) => {
  ...
  const handleNav = (id: string) => {
    ...
    navigate(routes[id] ?? '/patient/dashboard');
    setSidebarOpen(false);
  };

  // ✅ FIX 2: BookAppointment has its own full-screen Shell — render it outside the
  //    padded wrapper to avoid double-padding and scroll conflicts.
  const isFullBleed = activeTab === 'book-appointment' || activeTab === 'emergency';

  const renderContent = () => {
    switch (activeTab) {
      case 'dashboard':
        return <DashboardHome patient={patient} onNavigate={handleNav} />;
      case 'book-appointment':
        // ✅ FIX 3: Pass patient_unique_id (or id) — component has a safe fallback anyway
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex">
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden lg:ml-72">
        ...
        {/* ✅ FIX 4: Full-bleed tabs (BookAppointment / Emergency) get zero padding so
            the component's own Shell fills the space correctly. All other tabs keep
            the normal padded layout with the Emergency button. */}
        {isFullBleed ? (
          // No padding, no emergency button — BookAppointment manages its own chrome
          <div className="flex-1 overflow-y-auto">
            {renderContent()}
          </div>
        ) : (
          <div className="flex-1 overflow-y-auto p-4 lg:p-8">
            {/* Emergency button — only shown on non-full-bleed tabs */}
...
/* ── Dashboard Home ──────────────────────────────────────────────── */
const DashboardHome: React.FC<{ patient: Patient; onNavigate: (id: string) => void }> = ({
  patient,
  onNavigate,
}) => (
  <div className="space-y-6">
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div className="bg-white rounded-xl shadow-sm p-6 flex flex-col items-center justify-center min-h-[300px]">
        ...
        <p className="text-gray-500 text-center mb-6 max-w-sm">
          Start your health journey by booking your first appointment with our specialists.
        </p>
        {/* ✅ FIX 5: Wire the Book button on dashboard to actually navigate */}
        <Button
          onClick={() => onNavigate('book-appointment')}
          className="bg-blue-600 hover:bg-blue-700 text-white px-8 py-3 rounded-lg shadow-lg hover:shadow-xl transition-all transform hover:-translate-y-1"
        >
    ...
    <PatientChatbot patient={patient} onClose={() => { }} />
  </div>
);