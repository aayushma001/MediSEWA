...
export const PatientsView: React.FC<PatientsViewProps> = ({ doctorId }) => {
    ...
    useEffect(() => {
        const fetchPatients = async () => {
            try {
                // In real app, fetch from API. Using mock for demo.
                const mockData = [
                    {
                        id: 1,
                        user: { first_name: 'Adrian', last_name: 'Marshall', email: 'adrian@example.com', mobile: '+1 504 368 6874' },
                        condition: 'Chronic Cough',
                        lastVisit: '2024-01-20',
                        status: 'pending',
                        reports: [
                            { id: 'R1', date: '2024-01-20', type: 'Prescription', doctor: 'Dr. House', summary: 'Amoxicillin 500mg, Rest for 3 days' }
                        ]
                    },
                    {
                        id: 2,
                        user: { first_name: 'Kelly', last_name: 'Stevens', email: 'kelly@example.com', mobile: '+1 832 891 8403' },
                        condition: 'Hypertension',
                        lastVisit: '2023-12-15',
                        status: 'all',
                        reports: [
                            { id: 'R2', date: '2023-12-15', type: 'Lab Result', doctor: 'Dr. Strange', summary: 'Blood pressure 140/90. Suggested low salt diet.' },
                            { id: 'R3', date: '2023-11-01', type: 'Initial Checkup', doctor: 'Dr. Strange', summary: 'Baseline vitals recorded.' }
                        ]
                    },
                    {
                        id: 3,
                        user: { first_name: 'Samuel', last_name: 'Anderson', email: 'samuel@example.com', mobile: '+1 749 104 6291' },
                        condition: 'Back Pain',
                        lastVisit: '2024-02-01',
                        status: 'all',
                        reports: [
                            { id: 'R4', date: '2024-02-01', type: 'MRI Scan', doctor: 'Dr. Grey', summary: 'Minor lumbar disc protrusion.' }
                        ]
                    },
                ];
    ...
    const filteredPatients = patients.filter(p => {
        const matchesSearch = `${p.user.first_name} ${p.user.last_name}`.toLowerCase().includes(searchTerm.toLowerCase()) ||
                            ...
                            {filteredPatients.map((patient) => (
                                <tr key={patient.id} className="hover:bg-blue-50/30 transition-colors group">
                                    ...
                                    <td className="px-6 py-4">
                                        ...
                                        >
                                            <FileText size={16} />
                                            <span>{patient.reports.length} Records</span>
                                        </button>
                                    </td>
                                    ...
                                    {selectedReport.reports.map((r: any) => (
                                        <div key={r.id} className="p-4 border border-gray-100 rounded-2xl hover:border-blue-200 transition-all bg-white shadow-sm">
                                            <div className="flex justify-between items-start mb-2">
                                                <div>
                                                    <span className="text-xs font-bold text-blue-600 uppercase bg-blue-50 px-2 py-0.5 rounded">{r.type}</span>
                                                    <h5 className="font-bold text-gray-900 mt-1">{r.date}</h5>
                                                </div>
                                                <span className="text-xs text-gray-500">By {r.doctor}</span>
                                            </div>
                                            ...
                                            <div className="mt-3 flex space-x-3">
                                                ...
                                                <button className="text-xs font-bold text-gray-500 flex items-center hover:underline">
                                                    <ExternalLink size={14} className="mr-1" /> View Full Record
                                                </button>
                                            </div>
                                        </div>
                                    ))}
