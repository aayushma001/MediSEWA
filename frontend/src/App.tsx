...
function App() {
  ...
  // ✅ One place to define where each role should land after login
  const getDashboardPath = (u: User) => {
    if (u.user_type === "patient") return "/patient";
    if (u.user_type === "doctor") return "/doctor";
    if (u.user_type === "hospital") return "/hospital";
    return "/";
  };
  ...
  const handleLogin = async (data: LoginFormData) => {
    try {
      ...
      // ✅ Prevent blank page: move to correct dashboard immediately
      window.location.href = getDashboardPath(loggedInUser);
    } catch (err) {
  ...
  const handleRegister = async (data: RegisterFormData) => {
    try {
      ...
      // ✅ Prevent blank page
      window.location.href = getDashboardPath(registeredUser);
    } catch (err) {
  ...
  const handleLogout = () => {
    ...
    // optional: bring back to homepage
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener("open-auth-login", handler as EventListener);
  }, []);
                    ...
                    <div className="bg-white rounded-2xl shadow-2xl max-w-5xl w-full h-[90vh] overflow-hidden transform transition-all duration-300 scale-100 flex flex-col md:flex-row relative">
                      ...
                      <div className="hidden md:block w-1/2 bg-gray-50 relative overflow-hidden">
                        ...
                        <div className="absolute top-12 left-12 z-20 text-white">
                          <h2 className="text-4xl font-bold font-serif italic mb-4 drop-shadow-lg">
                            Medico Health
                          </h2>
    ...
    <Router>
      <div className="min-h-screen bg-gray-50">
        {user.user_type !== "hospital" &&
          ...
          user.user_type !== "patient" && <Header user={user} onLogout={handleLogout} />}

        {/* ✅ Never allow "/" to be blank when logged in */}
        <Routes>
          <Route path="/" element={<Navigate to={getDashboardPath(user)} replace />} />
          <Route
            path="/*"
            element={<AppRoutes user={user} onLogout={handleLogout} onUserUpdate={handleUserUpdate} />}
          />
        </Routes>
      </div>
    </Router>
